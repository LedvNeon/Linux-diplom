Vagrant.configure("2") do |config| #создаём конфигурацию для всех VM, используя сокращение config
  ...
  config.vm.define "webdmz" do |webdmz|
    ...
    webdmz.vm.provision "shell", inline: "yum -y update"
    webdmz.vm.provision "shell", inline: "yum -y install epel-release"
    webdmz.vm.provision "shell", inline: "yum -y install traceroute tcpdump net-tools"
